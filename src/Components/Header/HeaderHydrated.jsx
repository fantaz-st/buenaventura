...
/**
 * Client‑only component – attaches behaviour, animations & state.
 * It replaces <HeaderStatic/> on the client once JS is ready, ensuring
 * super‑fast first paint while retaining full interactivity afterwards.
 */
export default function HeaderHydrated() {
  ...
  /* Scroll‑driven header hide/show + background opacity. */
  useEffect(() => {
    const onScroll = () => {
  ...
  /* GSAP mobile‑menu intro animation. */
  useEffect(() => {
    ...
    tl.current.eventCallback("onComplete", () => setActive(true));
    tl.current.eventCallback("onReverseComplete", () => setActive(false));
  }, []);

  /* Play / reverse menu animation. */
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Overlay & panel animated via GSAP */}
